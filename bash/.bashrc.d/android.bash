...
android-run() {
    ...
    if [ -n "$avd" ]; then
        ...
        if [ -f "$config_file" ]; then
            ...
            sed -i 's/^showDeviceFrame=.*/showDeviceFrame=yes/' "$config_file" || echo "showDeviceFrame=yes" >> "$config_file"
            sed -i 's/^hw.gpu.enabled=.*/hw.gpu.enabled=yes/' "$config_file" || echo "hw.gpu.enabled=yes" >> "$config_file"
        fi
