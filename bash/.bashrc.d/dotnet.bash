...
# local log_file="$log_dir/aspire-watch.log"
 ...
 _process_logs() {
   ...
   /^(trace|debug|info|warn|error|crit): .*Resources\..*\[0\]$/ {
     ...
     # Create a brief message preview (first 50 chars)
     msg_preview = substr(clean_msg, 1, 50)
     if (length(clean_msg) > 50) msg_preview = msg_preview "..."

     # Add massive padding and hidden searchable content
     padding = "                                                                                                                                                                  "

     print indicator " " service_clean " " time " " msg_preview padding "\t" clean_msg
   }
 ' "$log_file" | tac
} 
